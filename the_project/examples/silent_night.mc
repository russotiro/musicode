...
(part "Trumpet"
    (staff (key A) (clef treble)
        (notes r/2. r/2. E4/4. (tuplet 3/2 C#5/16 C#5/16 C#5/16) E4/4)
        (ending 1 (notes (grace (notes D4[slurBegin]/8) C#4[slurEnd]/2.)))
        (barline repeatEnd)
        (ending 2 (notes C#4/2.))
        (break line)
        (symbol segno)
        (notes B4/2 B4./4 G#4[trill]/2. A4/2 A4/4 E4/2.)
        (D.S. al Coda)
        (barline double)
        (coda (notes F#4/2. F#4/2.))
        (barline final)))
...
(part "Violoncello"
    (staff (key A) (clef bass)
        ...
        (notes A3/2.)
        (notes G#4/2 G#4./4 E4/2. (grace (notes F#4/8) E4/2) E4/4 C#4/2.)
        (coda (notes F#3/2. F#3/2.)))
    (staff (key A) (clef bass)
        (notes A2/2. A2/2. A2/2. A2/2. A2/2. E3/2. E3/2. A2/2. C#3/2.)
        (coda (notes F#2/2. F#2/2.))))
(group end)